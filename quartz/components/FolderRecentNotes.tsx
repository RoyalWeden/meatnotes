...
export default ((userOpts?: Partial<Options>) => {
  ...
  const FolderRecentNotes: QuartzComponent = ({
    ...
    return (
      <div class={classNames(displayClass, "recent-notes")} id="recent-notes-folder">
        <div class="recent-notes-heading">
          <h3>{opts.title}</h3>
          <button class="section-collapse-btn recent-notes-collapse" aria-label="Toggle section" data-target="recent-notes-folder">
  ...
  FolderRecentNotes.afterDOMLoaded = `
(function() {
  var STORAGE_KEY = 'recent-notes-folder-collapsed'

  function init() {
    var wrapper = document.getElementById('recent-notes-folder')
    if (!wrapper) return
